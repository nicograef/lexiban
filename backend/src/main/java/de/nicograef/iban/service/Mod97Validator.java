 ...
 *
 * Extracted from IbanValidationService into its own class because:
 * 1. It's a clearly defined algorithm — a standalone domain concept.
 * 2. Isolating it makes testing trivial (no Spring context, no other
 * dependencies).
 * 3. The Single Responsibility Principle: one class, one job.
 *
 * TS equivalent: a pure function `isValidMod97(iban: string): boolean`
 * Go equivalent: `func IsValidMod97(iban string) bool` in a `validation`
 * package.
 *
 * See docs/iban.md for the full algorithm explanation.
 */
@Service
public class Mod97Validator {
     ...
     * 3. The resulting number must satisfy: number mod 97 == 1.
     *
