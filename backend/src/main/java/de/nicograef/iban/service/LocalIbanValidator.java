...
/**
 * Local IBAN validator — country-specific length check, Mod-97 check digit
 * verification, and
 * German BLZ → bank name lookup.
 *
 * <p>
 * Returns definitive failures (wrong length, bad Mod-97) immediately. Returns a
 * complete result
 * for known German banks. Returns {@code Optional.empty()} for unknown banks —
 * signaling that
...
public class LocalIbanValidator implements IbanValidator {
    ...
    /**
     * Demo subset of German BLZ → bank name mappings (production: Bundesbank BLZ
     * file).
     */
    ...
    /**
     * Expected IBAN length per country code (ISO 13616). Unlisted countries skip
     * the length check.
     */
