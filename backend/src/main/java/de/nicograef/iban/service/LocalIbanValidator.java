...
/**
 * Local IBAN validator — validates using country-specific length checks, Mod-97 check digit
 * verification, and a local German BLZ → bank name map.
 *
 * <p>This validator can only produce a complete result for IBANs whose bank it recognizes (BLZ in
 * KNOWN_BANKS). For unknown banks or non-German IBANs, it returns Optional.empty() — signaling that
 * another validator should try.
 *
 * <p>However, definitive failures (wrong country length, bad Mod-97) are always returned — those
 * checks are universal and don't need external confirmation.
 *
 * <p>Pipeline: 1. Country length check → wrong → definitive invalid 2. Mod-97 check digits → wrong
 * → definitive invalid 3. BLZ in KNOWN_BANKS? → hit → valid + bankName → miss → empty (another
 * validator should try)
 *
 * <p>TS analogy: a validation function that returns ValidationResult | null, where null means "I
 * can't handle this one, try another strategy."
 *
 * <p>Go analogy: func (v *LocalValidator) Validate(iban IbanNumber) (*ValidationResult, error)
...
public class LocalIbanValidator implements IbanValidator {
    ...
    /**
     * Demo subset of German BLZ → bank name mappings. In production, this would be loaded from the
     * Bundesbank BLZ file (~3,600 entries). Only relevant for German IBANs — other countries use
     * different bank identifier schemes (Sort Code in GB, BC-Nummer in CH, etc.).
     */
    ...
    /**
     * Expected IBAN length per country code (ISO 13616). The shortest IBAN is Norway (15), the
     * longest Malta (31), max possible is 34. Countries not listed here skip the length check —
     * only Mod-97 is applied. Source: iban.md §5 + Wikipedia list of IBAN formats.
     */
    ...
    /**
     * Constructor injection — only depends on the pure Mod-97 algorithm. No I/O, no repository, no
     * external services.
     */
     ...
     * @return Definitive invalid result, definitive valid+bankName, or empty if the bank is unknown
     *     (= another validator should try)
     */
    @Override
