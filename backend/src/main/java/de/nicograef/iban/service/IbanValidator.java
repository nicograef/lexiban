 ...
 *
 * Each implementation attempts to produce a complete ValidationResult
 * (including bank name for valid IBANs). Returns Optional.empty() if it
 * cannot handle the given IBAN (e.g. unknown BLZ, API unreachable).
 *
...
public interface IbanValidator {
     ...
     * @return Present with a definitive result, or empty if this validator
     *         cannot handle the IBAN (another validator should try)
     */
